...
export async function POST(req: NextRequest) {
  try {
    const { parableId, altitude } = await req.json();
    ...
    if (!altitude || !altitudeOrder.includes(altitude as AltitudeKey)) {
      console.error('Invalid or missing altitude');
      return NextResponse.json({ error: 'Missing or invalid altitude' }, { status: 400 });
    }
    ...
    console.log(`Generating note for ${altitude}...`);

    const altitudeConfig = altitudePromptConfig[altitude as AltitudeKey];
    if (!altitudeConfig) {
      console.error(`No prompt configuration found for altitude: ${altitude}`);
      return NextResponse.json({ error: 'Invalid altitude configuration' }, { status: 400 });
    }

    // Since we're processing one at a time, we don't need prior notes for context
    const userPrompt = buildAltitudePrompt({
      altitudeKey: altitude as AltitudeKey,
      parableTitle: parable.title,
      scriptureRefs,
      sharedPromptSection,
    });

    console.log(`Making OpenAI API call for ${altitude}...`);
    ...
    try {
      ...
      console.log(`OpenAI response for ${altitude}:`, content ? 'Success' : 'No content');
      ...
      if (error) {
        console.error(`Error inserting note for ${altitude}:`, error);
        throw error;
      }

      console.log(`Successfully inserted note for ${altitude}`);
      
      return NextResponse.json({ 
        success: true,
        altitude,
        message: `Successfully generated note for ${altitude} altitude`,
        data
      });
      
    } catch (error) {
      console.error(`Error processing ${altitude}:`, error);
      return NextResponse.json({ 
        success: false, 
        error: `Failed to generate note: ${error instanceof Error ? error.message : 'Unknown error'}` 
      }, { status: 500 });
    }
  } catch (error) {
    console.error('Error in generate-notes endpoint:', error);
    return NextResponse.json({ 
      success: false,
